package com.increment.meclip.stepDefinition;

import com.increment.meclip.config.CommonMethod;
import com.increment.meclip.config.ConfigFileReader;
import com.increment.meclip.config.ExcelHelpers;
import com.increment.meclip.config.WebDriverManager;
...
public class Steps extends WebDriverManager {
    ...
    private int sec = config.getViewSecond();
    ...
    public List<DataExport> open_url_video_and_wait_run_video_time(String sheetName, int sec, String phone) throws Exception {
        ...
        for(int i = 1; i <= sum_row ; i++){
            ...
            // Kiểm tra lại video có bị nhảy sang video khác không
            if (!driver.getCurrentUrl().equals(url)){  // Kiểm tra video bị next bài
                ...
                if (reload.contains("ERR_INTERNET_DISCONNECTED")){
                    ...
                    System.out.println(mess_error);
                    continue;
                } else if (src_video.equals("")){
                    mess_error = "Video bị pause";
                    common.clickElement(Video_Check);
                    if (src_video.equals("")){
                        mess_error = mess + "\n" + "Click video lần 1: Video vẫn bị pause";
                        common.clickElement(Video_Check);
                        if (src_video.equals("")){
                            mess_error = mess + "\n" +"Click video lần 2: Video vẫn bị pause -> out";
                            want_continue = true;
                        }
                    }
                    dataExportList.add(new DataExport(phone, url, view_1, view_2, mess_error));
...
//        WebElement name = driver.findElement(InputPhone);
    ...
    @Given("Run tool and video viewing time")
    public void run_tool_and_video_viewing_time() throws Exception {
        String sh_1 = "Account";
        String sh_2 = "Url";
